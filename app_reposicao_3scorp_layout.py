...
<style>
    .stApp {
        background: radial-gradient(circle at top left, #16202b 0%, #0b1118 35%, #070b10 100%);
        color: #f8fafc;
    }
    ...
    .top-header {
        display: grid;
        grid-template-columns: 285px 1fr 280px 280px 280px;
        gap: 14px;
        align-items: stretch;
        margin-bottom: 16px;
    }
...
def ler_csv_drawback_bytes(file_bytes):
    ...
    if "QUANTIDADE_AUTORIZADA" in df.columns:
...
def plot_bar(df, xcol, titulo, cor):
    base = df.copy()
    if base.empty or xcol not in base.columns:
        fig = go.Figure()
    else:
        base = base.sort_values(xcol, ascending=False).head(10).copy()
        base["DESCRICAO_GRAFICO"] = base["DESCRICAO_COMPLEMENTAR_AUTORIZADA"].astype(str).str[:32]
        fig = px.bar(
...
def plot_consumo(total_realizado, total_autorizado):
    consumido = (total_realizado / total_autorizado * 100) if total_autorizado else 0
    ...
    fig = go.Figure(
        data=[
            go.Pie(
                ...
                labels=["Valor Consumido", "Valor Disponível"],
                hole=0.62,
    ...
    fig.add_annotation(
        text=f"<b>{br_num(consumido)}%</b><br>CONSUMIDO",
        x=0.5,
        y=0.5,
        showarrow=False,
    ...
    fig.update_layout(
        ...
        legend=dict(orientation="v", y=0.5, x=0.78),
    )
    return fig
...
with st.sidebar:
    ...
    if df.empty and uploaded_file is not None:
        try:
            df, _, nome_arquivo = ler_arquivo_upload(uploaded_file)
...
with g1:
    ...
    st.plotly_chart(plot_bar(df_filtrado, "VALOR_SALDO_DBK", "Saldo por valor", "#e71d3c"), use_container_width=True)
    st.markdown("</div>", unsafe_allow_html=True)

with g2:
    st.markdown('<div class="chart-card"><div class="chart-title">Saldo por quantidade</div>', unsafe_allow_html=True)
    st.plotly_chart(plot_bar(df_filtrado, "QTDE_SALDO_DBK", "Saldo por quantidade", "#35c6c9"), use_container_width=True)
    st.markdown("</div>", unsafe_allow_html=True)

with g3:
    st.markdown('<div class="chart-card"><div class="chart-title">Percentual consumido</div>', unsafe_allow_html=True)
    st.plotly_chart(plot_consumo(total_valor_real, total_valor_aut), use_container_width=True)
    st.markdown("</div>", unsafe_allow_html=True)
...
column_config = {}
if "% CONSUMIDO (VALOR)" in tabela.columns:
    column_config["% CONSUMIDO (VALOR)"] = st.column_config.ProgressColumn(
        "% CONSUMIDO (VALOR)",
        help="Percentual calculado com base no valor realizado versus valor autorizado",
        format="%.2f%%",
        min_value=0,
        max_value=100,
    )
...
st.caption("* Percentual consumido calculado com base no valor realizado versus valor autorizado.")

st.markdown(
    """
    <div style="height:12px;"></div>
    <div class="small-muted">
        Observação: no Streamlit Cloud, o arquivo enviado fica salvo como última versão enquanto o aplicativo permanecer ativo.
        Para uso permanente e multiusuário, recomenda-se salvar os dados em banco de dados, Google Sheets, S3 ou outro armazenamento externo.
    </div>
    """,
    unsafe_allow_html=True,
)
