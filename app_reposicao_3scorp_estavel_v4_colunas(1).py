...
<style>
.stApp {
    background: radial-gradient(circle at top left, #17212c 0%, #0a1018 42%, #060a0f 100%);
    color: #f8fafc;
}
...
.card {
    ...
    background: linear-gradient(145deg, rgba(18,27,37,.96), rgba(10,16,23,.96));
    border-radius: 10px;
    padding: 16px;
...
[data-testid="stVerticalBlockBorderWrapper"] {
    ...
    background: linear-gradient(145deg, rgba(18,27,37,.96), rgba(10,16,23,.96)) !important;
...
def plot_bar(df, xcol, cor):
    ...
    if base.empty or xcol not in base.columns or "DESCRICAO_COMPLEMENTAR_AUTORIZADA" not in base.columns:
        ...
        base["DESCRICAO_GRAFICO"] = base["DESCRICAO_COMPLEMENTAR_AUTORIZADA"].astype(str).str[:38]
        fig = px.bar(
...
def preparar_tabela(df):
    ...
    renomear = {
        "NUMERO_ITEM_REPOSICAO": "NUMERO_ITEM",
        "DESCRICAO_COMPLEMENTAR_AUTORIZADA": "DESCRICAO_COMPLEMENTAR",
        "QUANTIDADE_AUTORIZADA": "QTD_AUTORIZADA",
        "QUANTIDADE_REALIZADA": "QTD_REALIZADA",
        "QTDE_SALDO_DBK": "SALDO_QTD",
        "% CONSUMIDO (QTD)": "% CONSUMIDO QTD",
        "VALOR_AUTORIZADO": "VALOR_AUTORIZADO",
        "VALOR_REALIZADO": "VALOR_REALIZADO",
        "VALOR_SALDO_DBK": "SALDO_VALOR",
        "% CONSUMIDO (VALOR)": "% CONSUMIDO VALOR",
    }
...
with st.sidebar:
    st.markdown(
        ...
        <div class="card logo-card">
            ...
            <div style="font-weight:900;text-transform:uppercase;margin-top:8px;">Reposição</div>
        </div>
        """,
        unsafe_allow_html=True,
    )

    st.markdown('<div class="card"><div class="header-label">Upload de arquivo</div><div class="small-muted">Envie o arquivo CSV ou ZIP exportado do Drawback.</div></div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="card"><div class="header-label">Arquivo atual</div>', unsafe_allow_html=True)
    if nome_arquivo:
    ...
    st.markdown('<div class="card"><div class="header-label">Filtros</div></div>', unsafe_allow_html=True)
...
h1, h2, h3, h4 = st.columns([1.4, 1.7, 1.7, 1.7])

with h1:
    st.markdown(
        f"""
        <div class="card" style="min-height:106px;display:flex;align-items:center;">
            <img src="data:image/png;base64,{LOGO_3S_BASE64}" style="max-width:210px;width:96%;">
        </div>
        """,
        unsafe_allow_html=True,
    )
...
with h2:
    st.markdown(
        ...
        <div class="card" style="min-height:106px;">
            <div class="main-title">REPOSIÇÃO</div>
            <div class="main-subtitle">Controle de Saldos de Drawback</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
with h4:
    st.markdown(
        ...
        <div class="card" style="min-height:106px;">
            ...
            <div class="header-sub">Situação: {DADOS_AC["situacao"]}<br>Atualização: {ultima}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
with g1:
    with st.container(border=True):
        st.markdown('<div class="chart-title">Saldo por valor (USD)</div>', unsafe_allow_html=True)
        st.plotly_chart(plot_bar(df_filtrado, "VALOR_SALDO_DBK", "#e71d3c"), use_container_width=True)
with g2:
    with st.container(border=True):
        st.markdown('<div class="chart-title">Saldo por quantidade</div>', unsafe_allow_html=True)
        st.plotly_chart(plot_bar(df_filtrado, "QTDE_SALDO_DBK", "#35c6c9"), use_container_width=True)

g3, g4 = st.columns(2)
with g3:
    with st.container(border=True):
        st.markdown('<div class="chart-title">Percentual consumido por valor</div>', unsafe_allow_html=True)
        st.plotly_chart(plot_consumo(total_valor_real, total_valor_aut, "VALOR"), use_container_width=True)
with g4:
    with st.container(border=True):
        st.markdown('<div class="chart-title">Percentual consumido por quantidade</div>', unsafe_allow_html=True)
...
for pcol in ["% CONSUMIDO QTD", "% CONSUMIDO VALOR"]:
...
if "NCM" in tabela.columns:
    column_config["NCM"] = st.column_config.TextColumn("NCM", width="small")

if "NUMERO_ITEM" in tabela.columns:
    column_config["NUMERO_ITEM"] = st.column_config.TextColumn("NUMERO_ITEM", width="small")

if "DESCRICAO_COMPLEMENTAR" in tabela.columns:
    column_config["DESCRICAO_COMPLEMENTAR"] = st.column_config.TextColumn(
